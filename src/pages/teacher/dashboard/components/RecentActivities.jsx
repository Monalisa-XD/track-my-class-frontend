      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pb-4 border-b border-slate-100">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 font-medium">Your latest teaching actions and module events</p>
          </div>
        </div>
        <button
          type="button"
        ...
        {activities.map((act) => {
          ...
          return (
            <div key={act.id} className="relative flex items-start gap-4 group">
              {/* Timeline Node */}
              <div className={`relative z-10 w-9 h-9 rounded-full flex items-center justify-center shrink-0 shadow-md transition-transform duration-200 group-hover:scale-110 ${act.iconBg || 'bg-blue-600 text-white'}`}>
              ...
              <div className="flex-1 p-4 rounded-xl bg-slate-50/70 border border-slate-200/60 hover:bg-white hover:border-slate-300 hover:shadow-md transition-all duration-200 hover:-translate-y-0.5 space-y-2">
                ...
                <div className="flex items-center justify-between flex-wrap gap-2">
                  <div className="flex items-center gap-2">
                    <h4 className="text-sm font-bold text-slate-800 tracking-tight group-hover:text-blue-600 transition-colors">{act.title}</h4>
                    <span className="text-[10px] font-bold px-2 py-0.5 rounded-md bg-slate-200/70 text-slate-700">{act.module}</span>
                  </div>
                ...
                {/* Footer */}
                <div className="flex items-center gap-1 text-[11px] font-semibold text-slate-400 pt-2 border-t border-slate-200/40">
