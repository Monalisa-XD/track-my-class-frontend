...
export default function StatisticsCards({ items = [] }) {
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-5 select-none">
      {items.map((card) => {
        const IconComponent = card.icon;
        const glowClass = GLOW_CLASSES[card.color] || GLOW_CLASSES.blue;
        const chipClass = CHIP_CLASSES[card.color] || CHIP_CLASSES.blue;
        ...
        return (
          ...
          >
            ...
            <div className="flex items-start justify-between">
              <div className={`w-12 h-12 rounded-xl flex items-center justify-center shadow-md transition-transform duration-200 group-hover:scale-110 ${card.iconBg}`}>
            ...
            <div className="mt-4 space-y-1">
              ...
              <h3 className="text-4xl font-black text-slate-900 tracking-tight leading-none">{card.value}</h3>
            </div>

            {/* Bottom: Subtitle + Status chip */}
            <div className="mt-3.5 pt-3 border-t border-slate-100 flex items-center justify-between text-xs gap-2">
              <span className="font-semibold text-slate-500 truncate">{card.subtitle}</span>
              <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-extrabold border leading-none shrink-0 ${chipClass}`}>
                {card.chip || 'Active'}
              </span>
            </div>
          </div>
