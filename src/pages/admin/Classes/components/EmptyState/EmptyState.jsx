...
import { School, Plus, RotateCcw } from 'lucide-react';
    ...
    <div className="flex flex-col items-center justify-center p-8 md:p-12 bg-white rounded-2xl border border-slate-200/80 shadow-ambient select-none text-center max-w-xl mx-auto space-y-5">
      ...
      <div className="space-y-1.5">
        <h3 className="text-lg font-bold text-slate-800 tracking-tight">
          {isSearchEmpty ? 'No Classes Found' : 'No Classes Configured'}
        </h3>
        <p className="text-sm text-slate-500 font-medium max-w-sm leading-relaxed">
          {isSearchEmpty
            ? 'Try adjusting your text search queries or category filters to find the class roster.'
            : 'Get started by creating your very first academic class division config in the directory.'}
        </p>
      </div>
      ...
      <div className="flex items-center gap-2.5 pt-1">
        {isSearchEmpty ? (
          ...
          >
            ...
            <span>Configure First Class</span>
          </button>
        )}
      </div>
