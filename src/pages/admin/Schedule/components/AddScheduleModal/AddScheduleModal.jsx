...
export default function AddScheduleModal({
  ...
  // Filter classes based on selected department and course
  const filteredClasses = CLASSES_LIST.filter(c => c.dept === department && c.course === course);
  const defaultClass = filteredClasses[0] || CLASSES_LIST[0];
  const [classObj, setClassObj] = useState(defaultClass);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      const firstClass = CLASSES_LIST.find(c => c.dept === 'CSE' && c.course === 'MCA') || CLASSES_LIST[0];
      setClassObj(firstClass);
      setSubject(SUBJECTS_LIST[0]);
      setTeacher(TEACHERS_LIST[0]);
  ...
  // Dynamically update class options and set default selected class
  useEffect(() => {
    const matched = CLASSES_LIST.filter(c => c.dept === department && c.course === course && c.sem === semester);
    if (matched.length > 0) {
      setClassObj(matched[0]);
    } else {
      const genericMatched = CLASSES_LIST.filter(c => c.dept === department && c.course === course);
      setClassObj(genericMatched[0] || CLASSES_LIST[0]);
    }
  }, [department, course, semester]);

  // Auto-generate Schedule ID prefix
  useEffect(() => {
    if (classObj) {
      const codePart = classObj.code.replace('CL-', '');
      const subjectIndex = SUBJECTS_LIST.indexOf(subject);
      const subCode = subjectIndex !== -1 ? 100 + subjectIndex + 1 : 101;
      setId(`SCH-${codePart}-${subCode}`.toUpperCase().replace(/\s+/g, ''));
    }
  }, [classObj, subject]);
  ...
  const handleSubmit = (e) => {
    ...
    const newErrors = {};
    if (!id.trim()) {
      newErrors.id = 'Schedule ID is required';
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg bg-white rounded-2xl border border-slate-200 shadow-2xl overflow-hidden transform transition-all duration-300 ease-out flex flex-col max-h-[90vh]">
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-4">
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">
              Generated Schedule ID <span className="text-red-500">*</span>
            </label>
            <input
              type="text"
              value={id}
              onChange={(e) => setId(e.target.value)}
              placeholder="e.g. SCH-CSE-MCA-101"
              className={`w-full px-3.5 py-2 text-xs bg-slate-50 border rounded-xl outline-none focus:bg-white focus:ring-4 transition-all duration-200 font-semibold ${
                errors.id ? 'border-red-300 focus:border-red-500 focus:ring-red-500/10' : 'border-slate-200 focus:border-blue-500 focus:ring-blue-500/10'
              } text-slate-800 font-mono`}
            />
            {errors.id && (
              <p className="text-xs font-bold text-red-500">{errors.id}</p>
            )}
          </div>
