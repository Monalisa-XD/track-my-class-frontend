...
export default function StudentStats({ students = [] }) {
  ...
  // Calculate unique semesters represented
  const uniqueSems = new Set(
    students
      .map(s => s.semester)
      .filter(sem => sem && sem.trim() !== '')
  ).size;
  ...
  const statCards = [
    ...
    {
      id: 'stat-semester-students',
      title: 'Active Semesters',
      value: uniqueSems,
      label: 'Semester tracks',
      icon: Calendar,
      color: 'indigo',
      trend: 'Recently admitted',
      trendType: 'success'
    }
  ];
