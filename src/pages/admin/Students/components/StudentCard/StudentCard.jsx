...
import { Eye, Edit2, Trash2, Calendar, Mail, Phone } from 'lucide-react';
...
const renderAvatar = (avatarUrl, name, sizeClass = "w-10 h-10 text-xs") => {
  if (avatarUrl) {
    return (
    ...
    <div className="md:hidden relative bg-white p-5 rounded-2xl border border-slate-200/80 shadow-ambient inner-highlight flex flex-col justify-between hover:shadow-ambient-hover hover:border-slate-300 transition-all duration-300 ease-out select-none overflow-hidden space-y-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-1.5 mt-1 flex-wrap">
              <span className="px-1.5 py-0.5 text-[9px] rounded-lg bg-blue-50 text-blue-600 border border-blue-100/60 font-mono font-bold leading-none">
                {student.regNo}
              </span>
              ...
              <span className="px-1.5 py-0.5 text-[9px] rounded-md bg-blue-50 text-blue-750 border border-blue-100 font-bold leading-none">
                {student.course}
              </span>
            </div>
          </div>
      ...
      <div className="grid grid-cols-2 gap-3.5 py-3 border-y border-slate-100 text-xs font-semibold text-slate-600">
        <div className="space-y-1">
          <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Semester & Section</span>
          <p className="text-slate-700 truncate">{student.semester} • {student.section}</p>
        </div>

        <div className="space-y-1">
          <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Admission Date</span>
          <p className="text-slate-700 truncate font-mono">{student.admissionDate}</p>
        </div>

        <div className="space-y-1.5 col-span-2 text-slate-500 font-medium">
          <div className="flex items-center gap-1.5">
            <Phone className="w-3.5 h-3.5 text-slate-400" />
            <span className="text-xs text-slate-700 font-semibold">{student.phone}</span>
          </div>
          <div className="flex items-center gap-1.5">
            <Mail className="w-3.5 h-3.5 text-slate-400" />
            <span className="text-xs text-slate-500 font-semibold truncate">{student.email}</span>
          </div>
        </div>
      </div>
