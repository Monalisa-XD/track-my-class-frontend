      ...
      <div className="relative w-full max-w-md bg-white rounded-2xl border border-slate-200 shadow-2xl overflow-hidden transform transition-all duration-300 ease-out flex flex-col">
        ...
        <div className="p-6 space-y-3">
          <p className="text-sm font-semibold text-slate-600 leading-relaxed">
            Are you sure you want to delete the student record for <span className="font-bold text-slate-800">{selectedStudent.name}</span> (<strong className="font-mono text-slate-900">{selectedStudent.regNo}</strong>)?
          </p>
          <div className="p-3.5 rounded-xl bg-red-50/50 border border-red-100 text-xs font-medium text-red-700 leading-relaxed">
            <strong>Warning:</strong> Deleting this student profile is permanent and will cascade modifications across their registered class attendance records, marks history, and course enrollments.
          </div>
        </div>
        ...
        <div className="flex items-center justify-end gap-2.5 p-5 border-t border-slate-100 bg-slate-50/50">
          ...
          <button
            type="button"
            onClick={() => onConfirm(selectedStudent)}
