    ...
    <div className="md:hidden relative bg-white p-5 rounded-2xl border border-slate-200/80 shadow-ambient inner-highlight flex flex-col justify-between hover:shadow-ambient-hover hover:border-slate-300 transition-all duration-300 ease-out select-none overflow-hidden space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3.5 py-3 border-y border-slate-100 text-xs font-semibold text-slate-600">
        <div className="space-y-1">
          ...
          <p className="text-slate-700 truncate">{teacher.designation}</p>
        </div>

        <div className="space-y-1">
          <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Experience</span>
          <p className="text-slate-700 truncate">{teacher.experience}</p>
        </div>

        <div className="space-y-1.5 col-span-2">
          <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Subjects Assigned</span>
          <div className="flex flex-wrap gap-1">
            {teacher.subjects && teacher.subjects.length > 0 ? (
              teacher.subjects.map((sub, idx) => (
                <span 
                  key={`${teacher.id}-sub-${idx}`} 
                  className="inline-flex px-1.5 py-0.5 text-[9px] font-bold rounded-md bg-blue-50/70 text-blue-700 border border-blue-100"
                >
                  {sub}
                </span>
              ))
        ...
        <div className="space-y-1.5 col-span-2 text-slate-500 font-medium">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-xs text-slate-700 font-semibold">{teacher.phone}</span>
          </div>
          <div className="flex items-center gap-1.5">
            <Mail className="w-3.5 h-3.5 text-slate-400" />
            <span className="text-xs text-slate-500 font-semibold truncate">{teacher.email}</span>
          </div>
        </div>
      </div>
