        ...
        <div className="w-screen max-w-md bg-white border-l border-slate-200/80 shadow-2xl flex flex-col transform transition-transform duration-300 ease-out animate-slide-in-right">
          ...
          <div className="flex items-center justify-between p-5 border-b border-slate-100 bg-slate-50/50">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h3 className="text-base font-bold text-slate-800 tracking-tight">Faculty Profile</h3>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="p-3 bg-slate-50/50 border border-slate-100 rounded-xl flex items-center gap-2.5">
                  ...
                  <div>
                    <span className="text-[9px] text-slate-400 font-bold block uppercase leading-none">Experience</span>
                    <strong className="text-xs text-slate-700 font-bold mt-0.5 block">{teacher.experience}</strong>
                  </div>
                </div>
                ...
                <div className="p-3 bg-slate-50/50 border border-slate-100 rounded-xl flex items-center gap-2.5">
                  ...
                  <div>
                    <span className="text-[9px] text-slate-400 font-bold block uppercase leading-none">Joining Date</span>
                    <strong className="text-xs text-slate-700 font-bold mt-0.5 block font-mono">{teacher.joiningDate}</strong>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="space-y-2.5">
                <div className="flex items-center gap-3 p-3 bg-slate-50/30 border border-slate-100 rounded-xl">
                  ...
                  <div>
                    ...
                    <a href={`mailto:${teacher.email}`} className="text-xs text-blue-600 font-bold hover:underline mt-0.5 block">{teacher.email}</a>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3 p-3 bg-slate-50/30 border border-slate-100 rounded-xl">
                  ...
                  <div>
                    ...
                    <a href={`tel:${teacher.phone}`} className="text-xs text-slate-700 font-bold hover:underline mt-0.5 block font-mono">{teacher.phone}</a>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="flex flex-col gap-2">
                {teacher.subjects && teacher.subjects.length > 0 ? (
                  teacher.subjects.map((sub, index) => (
                    <div 
                      key={`drawer-sub-${index}`} 
                      className="flex items-center gap-2.5 p-3 rounded-xl bg-blue-50/35 border border-blue-100/50 hover:bg-blue-50/50 transition-colors"
                    >
                  ...
                  <p className="text-xs italic text-slate-400 font-semibold p-2">No subjects currently assigned to this faculty member.</p>
                )}
              </div>
            </div>
